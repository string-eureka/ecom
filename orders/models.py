...
class Cart(models.Model):
    owner = models.OneToOneField(
        CustomerUser, on_delete=models.CASCADE, related_name="cart"
    )
    ...
    def savings(self):
        ...
        total_savings = sum(
            (item.item.item_price - item.item.selling_price) * item.quantity
...
class Order(models.Model):
    customer = models.ForeignKey(
        CustomerUser, on_delete=models.CASCADE, related_name="orders"
    )
...
class OrderItem(models.Model):
    ...
    item = models.ForeignKey(
        Item, on_delete=models.SET_NULL, related_name="sold_items", null=True
    )
    quantity = models.PositiveSmallIntegerField(default=1)
...
class Wishlist(models.Model):
    owner = models.OneToOneField(
        CustomerUser, on_delete=models.CASCADE, related_name="wishlists"
    )
